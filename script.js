// script.js - reads pre-fetched data from /data/*.json (generated by GitHub Action)
...
// Load NIFTY 500 symbols from nifty500.json (for autocomplete)
...
// Fetch stock data from local JSON file (generated by GitHub Action)
async function fetchStockData(symbol) {
    const url = `/data/${symbol}.json`;
    const res = await fetch(url);
    if (!res.ok) {
        if (res.status === 404) {
            throw new Error(`No cached data for ${symbol}. The daily update may not have run yet.`);
        }
        throw new Error(`Failed to load data: HTTP ${res.status}`);
    }
    const data = await res.json();
    if (!data || !data.length) throw new Error('Data file is empty');
    return data;
}
...
// ---------- Indicator helpers (identical to before) ----------
function getRSI(r) { if (r > 70) return "SELL"; if (r < 30) return "BUY"; return "NEUTRAL"; }
function getMACD(m,s) { return m > s ? "BUY" : m < s ? "SELL" : "NEUTRAL"; }
function getEMA(p,e) { return p > e ? "BUY" : p < e ? "SELL" : "NEUTRAL"; }
function getBB(c,u,l) { if (c > u) return "SELL"; if (c < l) return "BUY"; return "NEUTRAL"; }
function getStochRSI(s) { if (s > 80) return "SELL"; if (s < 20) return "BUY"; return "NEUTRAL"; }
function getCCI(c) { if (c > 100) return "SELL"; if (c < -100) return "BUY"; return "NEUTRAL"; }
function getMomentum(m) { return m > 0 ? "BUY" : m < 0 ? "SELL" : "NEUTRAL"; }
function getOBV(o) { if (o.length<2) return "NEUTRAL"; return o[o.length-1] > o[o.length-2] ? "BUY" : "SELL"; }
function getSuperTrend(st,close) { return close > st ? "BUY" : "SELL"; }
...
// Main analysis (reads local JSON)
async function analyzeStock(symbol) {
    ...
    try {
        ...
        let msg = err.message;
        if (msg.includes('No cached data')) msg = `📦 No data for ${symbol}. The daily GitHub Action may not have run yet.`;
        else msg = `⚠️ ${msg}`;
