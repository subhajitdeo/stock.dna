...
def process_ticker(ticker: str, session: requests.Session) -> Optional[Dict]:
    ...
    # Corrected call: passes closes for Chikou
    ichimoku_components = ichimoku(highs, lows, closes)
    ...
    # so we use latest close against latest known cloud values
    ...
    # -------------------------------------------------------------------------
    # Signal aggregation (EXACT same logic as original)
    # -------------------------------------------------------------------------
    buy_signals = 0
    sell_signals = 0
    total_indicators = 0
